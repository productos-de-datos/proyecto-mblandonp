"""
Módulo de pronóstico final de precios diarios.
-------------------------------------------------------------------------------
...
Función forecasts: identifica los indices de y_test y selecciona las columnas de fecha, precio y pronóstico
...
def best_score(scores):
    ...
    estimador_n = estimador_n[estimador_n["scores"] == estimador_n["scores"].max()]
...
def train_model_with_best_estimator(estimador_n, x_train, y_train):
    ...
    model_RF = RandomForestRegressor(n_estimators=estimador_n, random_state=12345)
    model_RF.fit(x_train, y_train)
    return model_RF
...
def make_forecasts():
    try:
        ...
        model_RF = train_model_with_best_estimator(estimador_n, x_train, y_train)
